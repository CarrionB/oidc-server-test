...
class Account {
  accountId: any;
  profile: any;
  constructor(id: any, profile: any) {
  ...
  async claims(use: any, scope: any) { // eslint-disable-line no-unused-vars
    ...
    return {
      sub: this.accountId || 'admin', // it is essential to always return a sub claim
  ...
  static async findAccount(ctx: any, id: any, token: any) { // eslint-disable-line no-unused-vars
    ...
    if (!store.get(id)) new Account(id, profile); // eslint-disable-line no-new
    console.log(new Account(id, profile))
